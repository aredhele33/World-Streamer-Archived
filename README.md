...
Zomming out                                                                                                       | Slow move (human speed)
:----------------------------------------------------------------------------------------------------------------:|:--------------------------------------------------------------------------------------------------------------:
...
Note that the cell buffers are comrpessed. It allow to save a lot of disk space for a near 0 cost at runtime (since buffer are small).
From my tests, a high compression rate allows us to reduce the size of the file by almost 10.
...
Most of the time, when you read a file, this a blocking operation and you have to wait until the function retunrs.
...
If there're cell to load that are not already loaded, or being loeded, the streamer will initiate an asynchronous read :
